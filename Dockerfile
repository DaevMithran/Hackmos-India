...
# then log output of file /code/bin/dmhackmosd
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/dmhackmosd \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/dmhackmosd | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/dmhackmosd /usr/bin/dmhackmosd
...
CMD ["/usr/bin/dmhackmosd", "version"]
