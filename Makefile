...
HTTPS_GIT := github.com/rollchains/dmhackmoschain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=dmhackmoschain \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=dmhackmosd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/dmhackmosd ./cmd/dmhackmosd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/dmhackmosd.exe ./cmd/dmhackmosd
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/dmhackmosd
...
draw-deps:
	...
	@goviz -i ./cmd/dmhackmosd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c dmhackmoschain --local -f chains.yaml
endif
...
set-testnet-configs:
	dmhackmosd config set client chain-id localchain-1
	dmhackmosd config set client keyring-backend test
	dmhackmosd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | dmhackmosd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | dmhackmosd keys add acc1 --recover`
